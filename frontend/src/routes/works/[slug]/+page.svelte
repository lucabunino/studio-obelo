...
{#if loaded}
	<article>
		<aside class="info" aria-label="Work information">
			<div class="content" in:typewriter|global={{ duration: 1200 }}>
		...
		<div class="blocks" role="list">
			{#each work.blocks ?? [] as block, i (block._key)}
				{#if block._type === 'mediaBlock'}
					<div class="block media-block {block.alignment}" class:first={i === 0} role="listitem" in:fade|global={{ delay: 200 + i * 100, duration: 300 }}>
				...
				{:else if block._type === 'textBlock'}
					<div class="block text-block" class:first={i === 0} role="listitem" in:fade|global={{ delay: 200 + i * 100, duration: 300 }}>
						{#if block.text}
				...
				{:else if block._type === 'workReference' && block.work}
					<div class="block reference-block" class:first={i === 0} class:marginBottom={!block.marginBottom} role="listitem" in:fade|global={{ delay: 200 + i * 100, duration: 300 }}>
			...
			{#if prev || next}
				<nav class="block related-block projects glass-2" aria-label="Other projects" use:obeloGrid={{cols: 6, rows: 8, loop: true}}>
						<div class="links">
							{#if prev}
